...
//		out.putField(Iso8583.Bit._007_TRANSMISSION_DATE_TIME, in.getTotalHexString().substring(beginIndex, endIndex));
		...
		//127.2 SWITCHKEY
		out.putPrivField(Iso8583Post.PrivBit._002_SWITCH_KEY, "0200".concat(Transform.fromEbcdicToAscii(Transform.fromHexToBin(in.getTotalHexString().substring(248, 268)))).concat(cons.substring(12, 16)));
		...
		if(out.getStructuredData() != null) {
			StructuredData sd = out.getStructuredData();
			sd.put("B24_Field_35", "009901".concat(Pack.resize(Transform.fromEbcdicToAscii(Transform.fromHexToBin(in.getTotalHexString().substring(182, 214))), 18, '0', false)).concat("D921200000001"));	
			out.putStructuredData(sd);
		}
		else {
			StructuredData sd = new StructuredData();
