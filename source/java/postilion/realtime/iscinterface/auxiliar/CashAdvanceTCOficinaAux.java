...
public class CashAdvanceTCOficinaAux {
	...
	public Iso8583Post processMsg(Iso8583Post out, ISCReqInMsg in, TransactionSetting tSetting, String cons, boolean enableMonitor, boolean isNextDay) throws XPostilion 
	{
		...
		try {
			...
			// tranType.concat(fromAccount).concat(toAccount));
			...
			if (naturalezaPago.equals("6") // si es AVANCE
					...
					) 
				{
					...
					// 127.22 TAG B24_Field_126 longitud total: 342 
					/* Tokens: 
					 * QT (longitud 42), B2 (longitud 168), B3 (longitud 90), B4 (longitud 30) 
					 * Ejemplo: (126)pos_preauth: & 0000500342! QT00032
						0130000000000000000000000000000 ! B200158
						7FF9000080808004080001781D35C8E468AB0000000000000000000000003C000027170170210505011347B646000706010A03A0B80100000000000000000000000000000000000000000000000000!
						B300080
						CF00SmartPOS60D8C8000000000000001100020204000007A0000000031010000000000000000000!
						B400020 05151000000000000000	*/
					token_QT = "! QT00032 0130000000000000000000000000000 ";
					...
					token_B4 = "! B400020 05151000000000000000";
					sd.put("B24_Field_126",
							"& 0000500342".concat(token_QT).concat(token_B2).concat(token_B3).concat(token_B4));
			...
			// }
			/*
			 * else { //Otros canales sd.put("Numero_Terminal",codigoOficina);
